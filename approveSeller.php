<?php

// Include database connection
$con = new mysqli("localhost", "Repair_Helper_DB_Server", "Madush@RepairHelper", "repair_helper");

if ($con->connect_error) {
    die("Connection failed: " . $conn->connect_error);
}


if (isset($_GET['id'])) {
    $Email_Address   = intval($_GET['id']); // For GET request
    $status = "Approve";
    
    // Prepare update query
    $sql = "UPDATE sellerdetails SET status = ? WHERE Email_Address  = ?";
    $stmt = mysqli_prepare($con, $sql);

    if ($stmt) {
        mysqli_stmt_bind_param($stmt, "si", $status, $Email_Address ); // "s" for string, "i" for integer

        if (mysqli_stmt_execute($stmt)) {
             header("Location:add_seller.php");

        } else {
            echo "<script>alert('Shipping Error: " . mysqli_stmt_error($stmt) . "');</script>";
        }

        mysqli_stmt_close($stmt);
    } else {
        echo "<script>alert('Error preparing statement');</script>";
    }
} else {
    echo "<script>alert('Error: Missing product_id');</script>";
}


?>
