...
if ($result->num_rows > 0) {
    ...
    echo '      <th style="width:20%;" scope="col">Seller_Email</th>';
    echo '      <th style="width:12%;" scope="col">price</th>';
    ...
    while ($row = $result->fetch_assoc()) {
        echo '<tr>';
        echo '<td>';
        echo '<p class="image-name"><strong>Product Id : ' . htmlspecialchars($row['id']) . '</strong></p>';
        echo '<td>';
        echo '</tr>';

        echo '<tr>';
        echo '  <td><img src="' . htmlspecialchars($row['path']) . '" alt="' . htmlspecialchars($row['name']) . '" width="80"></td>';
        echo '  <td><p class="image-name">' . htmlspecialchars($row['Names']) . '</p></td>';
        echo '  <td><p class="image-name">' . htmlspecialchars($row['catogory']) . '</p></td>'; // Fixed column
        echo '  <td><p class="image-name">' . htmlspecialchars($row['Seller_Email']) . '</p></td>'; // Fixed column
        echo '  <td><p class="image-name">Rs.' . htmlspecialchars($row['price']) . '.00</p></td>';
        echo '  <td><p class="image-name">' . htmlspecialchars($row['stock']) . '</p></td>';
        ...
        echo '  <td>
           <div class="dropdown">
              ...
              <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                  <li>
                      <a class="dropdown-item" href="productApprove.php?id=' . htmlspecialchars($row['id']) . '">Approve</a>
                  </li>
                  <li>
                      <a class="dropdown-item" href="rijectListing.php?id=' . htmlspecialchars($row['id']) . '" 
