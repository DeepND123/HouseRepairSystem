...
<?php
            ...
            if (isset($_GET['category'])) {
                ...
                $sql = "SELECT * FROM tec_profile Where status = 'Approve' and working_area = '$category'";
                $result = $conn->query($sql);


            } else {
                
                $sql = "SELECT * FROM tec_profile Where status = 'Approve'";
                $result = $conn->query($sql);
            }
            ...
            <?php
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                       ...
                       echo '<div class="address-card" onclick="window.location.href=\'bookAtec.php?profile_id=' . $row['profile_id'] . '&tec_email=' . $row['tec_email'] . '\'">';
 


                            echo '<p>Worker Name : - ' . htmlspecialchars($row['worker_name']) .'</p>';
                          
                            
                            echo '<table style="width: 100%;">';
                                echo '<tr>';
                                    echo '<td style="width: 30%;">';
                                        echo '<img style="width:200px;height:200px" class="product-image" src="' . htmlspecialchars($row['image_path']) . '" alt="' . htmlspecialchars($row['image_name']) . '">';
                                    echo '</td>';
                                    
                                    echo '<td style="width: 60%;">';
                                         echo '<h4>' . htmlspecialchars($row['workname']) . '</h4><br>';
                                         echo '<p><strong>Woking Area  :  </strong>' . htmlspecialchars($row['working_area']) . '</p>';
                                         echo '<p><strong>About My Work :  </strong>' . htmlspecialchars($row['description']) . '</p>';
                                        
                                         echo '<hr>';
                                         echo '<h4 style="color:orange;">Rates :  Rs.' . htmlspecialchars($row['hourly_rate']) . '.00</h4>';
                                    echo '</td>';
                    ...
                    echo "<p><strong>No Work listed yet...</strong></p>";
                }
                ?>
            </div>
            <div class="col-sm-2"></div>
