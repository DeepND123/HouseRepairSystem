...
<?php
if (isset($_GET['product_id'], $_GET['buyer_email'], $_GET['seller_email'], $_GET['quantity'], $_GET['total_amount'], $_GET['cart_id'])) {
    // Retrieve values from the URL parameters
    $product_id = urldecode($_GET['product_id']);
    $buyer_email = urldecode($_GET['buyer_email']);
    $seller_email = urldecode($_GET['seller_email']);
    $quantity = urldecode($_GET['quantity']);
    $total_amount = urldecode($_GET['total_amount']);
    $cart_id = urldecode($_GET['cart_id']);
    ...
    // Use prepared statement to fetch cart details safely
    $sql = "SELECT * FROM product_cart WHERE cart_id = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("i", $cart_id); // 'i' denotes an integer
    $stmt->execute();
    $cart_result = $stmt->get_result();

    if ($cart_result->num_rows > 0) {
        $cart = $cart_result->fetch_assoc(); // Fetch cart details
    } else {
        echo "<script>alert('Cart details not found!'); window.location.href='cart.php';</script>";
        exit;
    }
    $stmt->close();


    $sql = "SELECT * FROM userdetails WHERE Email_Address = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("s", $buyer_email); // 'i' denotes an integer
    $stmt->execute();
    $buyer_result = $stmt->get_result();

    if ($buyer_result->num_rows > 0) {
        $user = $buyer_result->fetch_assoc(); // Fetch cart details
    } else {
        echo "<script>alert('Cart details not found!'); window.location.href='cart.php';</script>";
        exit;
    }
    ...
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("i", $product_id); // 'i' denotes an integer
    $stmt->execute();
    $Product_result = $stmt->get_result();

    if ($Product_result->num_rows > 0) {
        $product = $Product_result->fetch_assoc(); // Fetch cart details
    } else {
        echo "<script>alert('Cart details not found!'); window.location.href='cart.php';</script>";
        exit;
    }
    ...
    echo "<script>alert('Missing required parameters!'); window.location.href='cart.php';</script>";
}
?>
    ...
    <div class="row">
        ...
        <div class="address-card">
            <p style="font-size :17px;font-weight: bold;">Product Details</p>
             ...
             <div style="padding-left: 50px;">
               <table>
                <tr>
                    <td style="width :10%">
                         <img class="product-image" src="<?php echo htmlspecialchars($cart['image_path']); ?>" alt="<?php echo htmlspecialchars($cart['image_name']); ?>">
                    </td>

                    <td style="width :55%;padding-left: 50px;">
                         <p><?php echo htmlspecialchars($cart['product_name']); ?></p>
                    </td>
                       
                    <td style="width :20%">
                         <p class="product-price">Rs. <?php echo htmlspecialchars($cart['total_amount']); ?>.00</p>
                    </td>
            ...
            <hr>
             ...
             <p>Sold By <?php echo htmlspecialchars($cart['seller_name']); ?></p>
          ...
          <table style="width :100%">
            ...
            <!---updating product quantity---->
             ...
             <form action="CartCheckout.php" method="POST">
                ...
                <input type="hidden" name="seller_email" value="<?php echo htmlspecialchars($cart['seller_email']); ?>">
                <input type="hidden" name="seller_name" value="<?php echo htmlspecialchars($cart['seller_name']); ?>">
